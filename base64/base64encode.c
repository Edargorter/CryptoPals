...
char* hex_to_chars(char* hex_string, int n)
{
...
//	printf("Old length: %ld New length: %ld\n", s_length, b64_len);
	...
	for(s_offset = 0, e_offset = 2; count < s_length;){
	...
	memset(enc, 0x3D, b64_len - e_count);
...
/**
	...
	char* bytes = hex_to_chars(test, n);
